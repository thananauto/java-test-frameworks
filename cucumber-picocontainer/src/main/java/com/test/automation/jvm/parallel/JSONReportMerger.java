package com.test.automation.jvm.parallel;
        ...
        for (File report : existingReports) {
        	...
        	if (report.getName().endsWith(".json")) {
                ...
                if (mergedReport == null) {
                    // copy just the cucumber.json  copy the base .json file in to cucumber.json file
                 // FileUtils.copyFileToDirectory(report, reportDirectory);
                	
                	 mergedReport=new File(reportDirectory+File.separator+reportFileName);
                	if(mergedReport.exists())
                		mergedReport.createNewFile();
                	
                	FileUtils.copyFile(report, mergedReport);
                  // File fileName=new File("target\\cucumber-parallel-report");
                	// FileUtils.copyFileToDirectory(report, fileName);
                    // access this first copied report
                	//mergedReport = new File(newJsonReport, reportFileName);
                   // mergedReport = new File(newJsonReport, reportFileName);
                } else {
